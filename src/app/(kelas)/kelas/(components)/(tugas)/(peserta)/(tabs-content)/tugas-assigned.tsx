...
export default function TugasAssigned() {
  const session = useSession();
  const initialData = useContext(TugasPesertaContext).tugasAssigned;
  ...
  switch (tugasAssigned.data?.length) {
    case 0:
      return (
        <AnimateSection
    ...
    default:
      return tugasAssigned.data.map((tugas, index) => (
        <AnimateSection
          key={tugas.id}
